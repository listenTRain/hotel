layui.use(['jquery','layer','table','form','laydate'], function() {
    ...
    $("#vip_num").blur(function () {
        ...
        if(vipNum.match('^\\d{16}$') && vipNum.length==16){
            layer.tips('会员卡号正确。。', this, {tips: [2,'green'],time:2000});
            //发送请求查询会员数据
            loadVipByVipNum(vipNum);
        }else {
            layer.tips('会员卡号为16位数字！！', this, {tips: [2,'#ce48fc'],time:2000});
        }
    });
    //自定义表单验证
    form.verify({
    ...
    form.on('submit(demo1)', function(data){
        ...
        //saveJsonInRoomInfo['isVip'] = $("input[name=isVip]").val();
        saveJsonInRoomInfo['status'] = '1';//显示
        saveJsonInRoomInfo['outRoomStatus'] = '0';//未退房
        //添加入住信息数据
        //1.完成入住信息的添加
